...
df_wide<-dplyr::select(df_wide, folio, conf_inter_1, conf_inter_2, conf_inter_3, conf_inter_4,
                ...
                quant_1, quant_2, quant_3, quant_4, indigena_es, sipw)
...
fit_M6 <- sem(M6, data = df_wide, missing = "fiml", sampling.weights = "sipw")
...
fit_M7 <- sem(M7, data = df_wide, missing = "fiml", sampling.weights = "sipw")
...
fit_M8 <- sem(M8, data = df_wide, missing = "fiml", group = "indigena_es", sampling.weights = "sipw")
...
print(doc, target = "Output/sensitivity analysis/model_fit_indices_table.docx")
...
main_table <- time_constrained_estimates %>%
  dplyr::select(group, effect_type, effect_label, Estimate, SE, Std_Estimate, p) %>%
...
print(doc, target = "Output/sensitivity analysis/model8_time_constrained__multi_estimates.docx")
